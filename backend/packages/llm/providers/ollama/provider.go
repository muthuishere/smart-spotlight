...
func (p *Provider) CreateMessage(
	...
	slog.Debug("sending messages to Ollama",
...
func (p *Provider) CreateToolResponse(
	...
	case string:
		contentStr = v
		slog.Debug("using string content directly")
	default:
		bytes, err := json.Marshal(v)
		if err != nil {
			slog.Error("failed to marshal tool response",
				"error", err,
				"content", content)
			return nil, fmt.Errorf("error marshaling tool response: %w", err)
		}
		contentStr = string(bytes)
		slog.Debug("marshaled content to JSON string",
			"result", contentStr)
	}
	...
	slog.Debug("created tool response message",
		"role", msg.GetRole(),
		"content", msg.GetContent(),
