...
// Message implements the models.Message interface
type Message struct {
	Msg APIMessage
}
...
func (m *Message) GetContent() string {
	slog.Debug("getting content from message", "message", m)

	var content []string
	for i, block := range m.Msg.Content {
		slog.Debug("processing content block", "index", i, "block", block)

		if block.Type == "text" {
			slog.Debug("adding text block", "text", block.Text)
			content = append(content, block.Text)
		} else if block.Type == "tool_result" {
			slog.Debug("processing tool result block", "block", block)
			...
			if len(content) == 0 && block.Text != "" {
				slog.Debug("falling back to direct text", "text", block.Text)
				content = append(content, block.Text)
			}
		}
	}
	...
	slog.Debug("final content result", "content", result)
	return result
}

func (m *Message) GetToolCalls() []models.ToolCall {
...
// ToolCall implements the models.ToolCall interface
